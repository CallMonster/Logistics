...
public class WheelView extends View {
     ...
     * wheel背景颜色
     */
    private int wheelBackground =R.color.colorWhite;
    /**
     * 显示多少个内容
     */
    ...
    public WheelView(Context context,int selectedIndex){
        super(context);
        this.context = context;
        initData();
...
//        Drawable drawable = new ColorDrawable(context.getResources().getColor());
//        //Drawable drawable =  new LinearGradientDrawable( GradientDrawable.Orientation.LEFT_RIGHT, mBgColors, mBgPositions );
//        super.setBackground(drawable);
        super.setBackgroundColor(wheelBackground);
    }
    ...
    private synchronized void drawList(Canvas canvas) {
        ...
        try {
            for(int i = 0;i<itemList.size();i++) {
                ItemObject itemObject = itemList.get(i);
                itemObject.drawSelf(canvas, getMeasuredWidth(),i);
            }
        } catch (Exception e) {
...
//                    break;
            ...
            public void run() {
                ...
                for (int j = 0;j<itemList.size();j++) {
                    if (itemList.get(j).isSelected()) {
                        if (onSelectListener != null){
                            selectedIndex = j;
                            onSelectListener.selecting(itemList.get(j).id, itemList.get(j).itemText);
                            break;
                        }
                    }
                }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        wheelBackground = attribute.getColor(R.styleable.WheelView_wheelAllBackground,0);
        attribute.recycle();
    ...
    private class ItemObject {
         ...
         */
        public void drawSelf(Canvas canvas, int containerWidth,int currentIndex) {
         ...
         * @return
         */
        public boolean isInView(int currentIndex) {
            boolean flag = false;
            if (selectedIndex==0&&(currentIndex==itemList.size()-1||currentIndex==0||currentIndex==1)){
                flag = true;
            }else if(selectedIndex==itemList.size()-1&&(currentIndex==itemList.size()-1||currentIndex==0||currentIndex==itemList.size()-2)){
                flag = true;
            }else if(currentIndex==selectedIndex||currentIndex == selectedIndex+1||currentIndex==selectedIndex-1){
                flag = true;
            }
            return flag;
        }
