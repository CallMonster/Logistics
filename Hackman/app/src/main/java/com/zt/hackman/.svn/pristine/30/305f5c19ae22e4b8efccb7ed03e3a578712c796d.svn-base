package com.zt.hackman.customshape;
...
public class SVGParser {
	 ...
	 */
	public static SVG getSVGFromInputStream(InputStream svgData,
			int searchColor, int replaceColor, int targetWidth, int targetHeight)
			throws SVGParseException {
	...
	private static SVG parse(InputStream in, Integer searchColor,
			Integer replaceColor, boolean whiteMode, int targetWidth,
			int targetHeight) throws SVGParseException {
		...
		try {
			...
			SVGHandler handler = new SVGHandler(picture, targetWidth,
					targetHeight);
	...
	private static class SVGHandler extends DefaultHandler {
		...
		private boolean doFill(Properties atts,
				HashMap<String, Shader> gradients) {
		...
		/**
		 * Start recording picture on the canvas. If target width and height are
		 * set for the canvas scale output picture uniformally using by the
		 * smallest dimention.
		 * 
		 * @param imageWidth
		...
		private Canvas beginRecordingPicture(int imageWidth, int imageHeight) {
			if (targetWidth == 0 || targetHeight == 0) {
				...
				Canvas canvas = picture.beginRecording(targetWidth,
						targetHeight);
		...
		private static final void prepareScaledCanvas(Canvas canvas,
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (localName.equals("svg")) {
				...
				if (centerX != null && centerY != null && radiusX != null
					...
					&& (localName.equals("polygon") || localName
							.equals("polyline"))) {
		...
		public void endElement(String namespaceURI, String localName,
			...
			if (localName.equals("svg")) {
				...
				if (gradient.id != null) {
					...
					LinearGradient g = new LinearGradient(gradient.x1,
				...
				if (gradient.id != null) {
					...
					RadialGradient g = new RadialGradient(gradient.x,
							gradient.y, gradient.radius, colors, positions,
