...
public class ChooseCardTypeWindow extends PopupWindow implements View.OnClickListener{
    ...
    public ChooseCardTypeWindow(Context context, ArrayList<String> banks,ArrayList<String> cards){
        super(context);
    ...
    private void init(){
        ...
        view = LayoutInflater.from(context).inflate(R.layout.window_choose_card_type, null);
        initView();
        setContentView(view);
    ...
    private void initView(){
        model = new CardTypeWindowModel();
        model.findViewByIds(view,this);
        model.initView(banks,cards);
...
//        });
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.window_card_cancel_btn:
                dismiss();
                break;
            case R.id.window_card_confirm_btn:
//                if(index == -1){
