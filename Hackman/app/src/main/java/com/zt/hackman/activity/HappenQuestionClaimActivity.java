...
public class HappenQuestionClaimActivity extends BaseActivity implements LeftListener,RightListener{
    ...
    protected void findViewByIds() {
        ...
        if(bundle!=null&&bundle.getString(Constant.INTNENTS.INTENT_ORDER_ID)!=null){
            if(bundle.getInt(Constant.INTNENTS.INTENT_ORDER_STATUS)==3){
                navModel.setTitle("取货遇到问题");
            }else if(bundle.getInt(Constant.INTNENTS.INTENT_ORDER_STATUS)==4){
                navModel.setTitle("交货遇到问题");
            }else{
                navModel.setTitle("响应数据有误");
            }
            model.findViewByIds(this,1,bundle.getString(Constant.INTNENTS.INTENT_ORDER_ID)
                    ,bundle.getInt(Constant.INTNENTS.INTENT_ORDER_STATUS),
                    bundle.getString("start_addr"),bundle.getString("end_addr"));
        }
    }
